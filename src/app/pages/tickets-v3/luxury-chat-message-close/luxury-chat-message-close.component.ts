...
@Component({
  selector: 'app-luxury-chat-message-close',
  templateUrl: './luxury-chat-message-close.component.html',
  standalone: true,
  imports: [LuxuryAppComponentsModule, CustomInputModule, CommonModule],
})
export default class LuxuryChatMessageCloseComponent implements OnInit {
  ...
  onLoadData() {
    const urlApi = `LuxuryChatMessage/GetByClosed/${this.id}`;
  ...
  onSubmit(): void {
    if (this.form.valid) {
      ...
      this.apiRequestService
        .onPut(`LuxuryChatMessage/Closed/${this.id}`, formData)
        .then((result: boolean) => {
