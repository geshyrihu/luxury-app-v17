...
@Component({
  selector: 'app-luxury-chat-participants',
  templateUrl: './luxury-chat-participants.component.html',
  standalone: true,
  imports: [
    CommonModule,
...
export default class LuxuryChatParticipantsComponent
  implements OnInit, OnDestroy
{
  ...
  form = this.formBuilder.group({
    ...
    luxuryChatGroupId: [this.config.data.id, Validators.required],
    applicationUserId: ['', Validators.required],
    applicationUser: ['', Validators.required],
    isAdmin: [false, Validators.required],
  });
  ngOnInit() {
    this.form.reset(); // Asegúrate de limpiar el formulario al iniciarlo
    this.onLoadAppUsers();
    this.onLoadExistingParticipants();
  }

  onLoadAppUsers() {
    const urlApi = `LuxuryChatParticipants/Participants/${this.customerIdService.getCustomerId()}/${
      this.config.data.id
    }`;
  ...
  onLoadExistingParticipants() {
    ...
    const urlApi = `LuxuryChatParticipants/ExistingParticipants/${this.config.data.id}`;
    this.apiRequestService.onGetList(urlApi).then((result: any) => {
  ...
  onSubmit(): void {
    ...
    if (this.id === '') {
      this.apiRequestService
        .onPost(`LuxuryChatParticipants`, this.form.value)
        .then((result: boolean) => {
          this.onCleanForm();
        });
    } else {
      this.apiRequestService
        .onPut(`LuxuryChatParticipants/${this.id}`, this.form.value)
        .then((result: boolean) => {
          this.onCleanForm();
        });
    }
  ...
  onEditParticipant(item: any) {
    ...
    this.form.patchValue({
      id: item.id,
      luxuryChatGroupId: item.luxuryChatGroupId,
  ...
  onDelete(id: number) {
    this.apiRequestService
      .onDelete(`LuxuryChatParticipants/${id}`)
      .then((result: boolean) => {
        if (result)
  ...
  onCleanForm() {
    ...
    this.form.reset();
    this.form.patchValue({
      luxuryChatGroupId: this.config.data.id,
    });
    this.ngOnInit();
